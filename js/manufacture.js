window.addEventListener("DOMContentLoaded", (event) => {
    ...
    if(manufacture_delete_button)
        manufacture_delete_button.addEventListener("click", delete_maufacture);
    
});

function delete_maufacture(){
    let manufacture_delete_button = document.getElementById('manufacture_delete_button');
...
function open_delete_modal(item){
    ...
    document.querySelector('#modal_delete_table .uk-modal-title span').innerHTML =  item.getAttribute('data-name') + ' (' +  item.getAttribute('data-country') + ')';
