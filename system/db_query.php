...
function get_manufactures_count($args){
    global $connection;
...
function get_orders_count($args){
    global $connection;
    $sql = "SELECT COUNT(*) as count FROM (SELECT orders.* FROM `orders` LEFT JOIN `clients` ON client = clients.id GROUP BY orders.id) as d";
    if($args['search']){
        $sql .= " WHERE d.title LIKE '%" . $args['search'] . "%' OR d.district LIKE '%" . $args['search'] . "%'  OR d.type LIKE '%" . $args['search'] . "%'";
    }
    $result = mysqli_fetch_assoc(mysqli_query($connection, $sql));
